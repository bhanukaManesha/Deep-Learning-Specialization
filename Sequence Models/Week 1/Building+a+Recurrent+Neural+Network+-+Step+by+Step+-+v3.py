...
def rnn_forward(x, a0, parameters):
    ...
    a_next = np.tanh(np.dot(parameters["Waa"],a0) + np.dot(parameters["Wax"],x[:,:,0]) + parameters["ba"])
    a[:,:,0] = a_next
...
def rnn_cell_backward(da_next, cache):
    ...
    dtanh = 1 - np.tanh(a_prev)**2

    # compute the gradient of the loss with respect to Wax (≈2 lines)
    dxt = np.dot(Wax.T,(1 - np.tanh(np.dot(Wax,xt) + np.dot(Waa,a_prev) + b)**2))
    dWax = np.dot((1 - np.tanh(np.dot(Wax,xt) + np.dot(Waa,a_prev) + b)**2),xt.T)

    # compute the gradient with respect to Waa (≈2 lines)
    da_prev = np.dot(Waa.T,(1 - np.tanh(np.dot(Wax,xt) + np.dot(Waa,a_prev) + b)**2))
    dWaa = np.dot((1 - np.tanh(np.dot(Wax,xt) + np.dot(Waa,a_prev) + b)**2),a_prev.T) 

    # compute the gradient with respect to b (≈1 line)
    dba = np.sum((1 - np.tanh(np.dot(Wax,xt) + np.dot(Waa,a_prev) + b)**2),axis=1 )
